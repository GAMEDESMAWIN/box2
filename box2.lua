...
do
	function a.a()
		...
		b.Clone = function(c)
			...
			if type(c) == "function" then
				d = clonefunction and clonefunction(c)
			elseif typeof(c) == "Instance" then
				d = cloneref and cloneref(c)
			end
			return d or c
		end
		b.ProtectUI = function(c)
			local d, e = pcall(function()
				return gethui()
			end)
			local f, g = pcall(function()
				local f = b.Clone(game:GetService("CoreGui"))
				if f and f.ClassName then
					return f
				end
				return
			end)
			local h, i = pcall(function()
				c.Parent = d and e or f and g or b.Clone(game:GetService("Players")).LocalPlayer.PlayerGui
			end)
			return h and c or i
		end
		return b
	...
	function a.c()
		local b = {}
		function b.Apply(c, d, e)
			for f, g in pairs(c) do
				if e and table.find(e, f) then
					continue
				end
				pcall(function()
					d[f] = g
		...
		function b.ReactiveTable(c, d)
			local e, f = table.clone(c or {}), {}
			local g = {
			...
			return setmetatable(f, g)
		end
		function b.Wrap(c, d, e, f)
			local g = {}
			setmetatable(g, {
				...
				__newindex = function(h, i, j)
					...
					if k then
						k(j)
						if not (f and table.find(f, i)) then
							c[i] = j
						end
					elseif e then
	...
	function a.d()
		...
		function d.Create(e)
			return function(f)
				...
				for h, i in pairs(f) do
					if h == "__dynamicKeys" and type(i) == "table" then
						for j, k in pairs(i) do
							pcall(function()
								g[j] = k.Value
							end)
							k:Connect(function(l)
								task.defer(pcall, function()
									if f.__contextKeys and f.__contextKeys._general then
										f.__contextKeys._general()
									end
									g[j] = f.__contextKeys and f.__contextKeys[j] and f.__contextKeys[j]() or l
								end)
							end)
						end
						continue
					end
					if typeof(i) == "table" and i.__unique then
						i.Parent = g
					end
					pcall(function()
						g[h] = i
	...
	function a.e()
		local b = a.load("a")
		local c = function(c)
			return b.Clone(game:GetService(c))
		end
		return {
			HttpService = (c("HttpService")),
			TweenService = (c("TweenService")),
			RunService = (c("RunService")),
			UserInputService = (c("UserInputService")),
			GuiService = (c("GuiService")),
	...
	function a.g()
		...
		return {
			...
			Controls = {
				...
				Selection = d("160deb", 100),
				SelectionStroke = d("160deb", 60),
				SelectionFocused = d("160deb", 100),
				SelectionFocusedAccent = d("FFFFFF", 85),
				Sidebar = d("202023", 84),
				Separator = {
					Background = d("000000", 50),
					Shadow = d("FFFFFF", 0),
				},
				Titlebar = d("363636", 100),
				TitlebarShadow = {
					Background = d("000000", 0),
					Color = c(ColorSequence.new({
						ColorSequenceKeypoint.new(0, Color3.fromRGB(0, 0, 0)),
						ColorSequenceKeypoint.new(1, Color3.fromRGB(255, 255, 255)),
					})),
					Transparency = c(NumberSequence.new({
				...
				MenuButton = { IndicatorBackground = d("FFFFFF", 10), MenuBackground = d("2C2C2E", 95) },
			},
		}
	end
	function a.h()
		...
		return {
			...
			Controls = {
				...
				TitlebarShadow = {
					...
					Transparency = c(
						NumberSequence.new({ NumberSequenceKeypoint.new(0, 0.35), NumberSequenceKeypoint.new(1, 0.35) })
					),
				},
				Toggle = {
					Knob = d("FFFFFF", 100),
	...
	function a.i()
		return { Dark = a.load("g"), Light = a.load("h") }
	end
	function a.j()
		local b, c = a.load("e"), a.load("d")
		local d, e, f, g =
			b.Workspace, b.Lighting, c.Create, function(d, e, f, g, h)
				return (d - e) * (h - g) / (f - e) + g
			end
		local h, i =
			function(h, i)
				local j = d.CurrentCamera:ScreenPointToRay(h.X, h.Y)
				return j.Origin + j.Direction * i
			end, function()
				local h = d.CurrentCamera.ViewportSize.Y
				return g(h, 0, 2560, 8, 56)
			end
		local j = function(j)
			local k, l, m, n, o =
				Instance.new("Folder", d.CurrentCamera),
				{},
				...
				(f("Part")({
					...
					CastShadow = false,
					Transparency = 0.98,
					f("SpecialMesh")({ MeshType = Enum.MeshType.Brick, Offset = Vector3.new(0, 0, -1E-6) }),
				})),
				f("DepthOfFieldEffect")({ FarIntensity = 0, NearIntensity = 1, InFocusRadius = 0.1, Parent = e })
			local p = n:FindFirstChildWhichIsA("SpecialMesh")
			j = j or 0.001
			local q, r =
				function(q, r)
					m.topLeft = r
					m.topRight = r + Vector2.new(q.X, 0)
					m.bottomRight = r + q
				end, function()
					local q = d.CurrentCamera
					local r, s, t, u = if q then q.CFrame else CFrame.identity, m.topLeft, m.topRight, m.bottomRight
					local v, w, x = h(s, j), h(t, j), h(u, j)
					local y, z = (w - v).Magnitude, (w - x).Magnitude
					n.CFrame = CFrame.fromMatrix((v + x) / 2, r.XVector, r.YVector, r.ZVector)
					if p then
						p.Scale = Vector3.new(y, z, 0)
					end
				end
			local s, t =
				function(s)
					...
					l[#l + 1] = s:GetPropertyChangedSignal("CFrame"):Connect(r)
					l[#l + 1] = s:GetPropertyChangedSignal("ViewportSize"):Connect(r)
					l[#l + 1] = s:GetPropertyChangedSignal("FieldOfView"):Connect(r)
					task.spawn(r)
				end
			n.Parent = k
		...
		return function(k, l)
			...
			k:GetPropertyChangedSignal("Visible"):Connect(function()
				m.SetVisibility(k.Visible)
			end)
			k.Destroying:Connect(function()
				o:Destroy()
			end)
			m.Model = o
			n(k)
	...
	function a.k()
		return { UIBlur = a.load("j") }
	end
	function a.l()
		return function(b)
			local c, d = a.load("d"), a.load("e")
			local e, f, g = c.Create, d.TweenService, {}
			g.Body = (
				e("Frame")({
					Name = "TextField",
					...
					e("UICorner")({ Name = "UICorner", CornerRadius = UDim.new(0, 5) }),
					e("UIStroke")({
						Name = "UIStroke",
						ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
						Thickness = 1,
						__dynamicKeys = {
							Color = b.Theme.Controls.ViewBorder[1],
							Transparency = b.Theme.Controls.ViewBorder[2],
						},
					}),
					e("TextBox")({
						Name = "Field",
						...
						TextSize = 15,
						__dynamicKeys = {
							PlaceholderColor3 = b.Theme.Text.Tertiary[1],
						...
						__contextKeys = {
							_general = function()
								...
								if h.Text == "" then
									h.PlaceholderColor3 = b.Theme.Text.Tertiary[1].Value
									h.TextTransparency = b.Theme.Text.Tertiary[2].Value
								else
									h.TextColor3 = b.Theme.Text.Primary[1].Value
									h.TextTransparency = b.Theme.Text.Primary[2].Value
								end
							end,
							TextTransparency = function()
								...
								return g.Field.Text == "" and b.Theme.Text.Tertiary[2].Value
									or b.Theme.Text.Primary[2].Value
							end,
						},
						e("UIPadding")({ Name = "UIPadding", PaddingTop = UDim.new(0, 4), PaddingBottom = UDim.new(
							0,
							4
						) }),
					}),
					e("UIPadding")({ Name = "UIPadding", PaddingLeft = UDim.new(0, 6), PaddingRight = UDim.new(0, 6) }),
					e("UIListLayout")({
						Name = "UIListLayout",
			...
			g.Stroke = (g.Body:FindFirstChild("UIStroke"))
			g.Padding = (g.Body:FindFirstChild("UIPadding"))
			g.Corner = (g.Body:FindFirstChild("UICorner"))
			g.Layout = (g.Body:FindFirstChild("UIListLayout"))
			g.Field = (g.Body:FindFirstChild("Field"))
			g.FieldPadding = (g.Field:FindFirstChild("UIPadding"))
			local h, i, j =
				TweenInfo.new(0.13, Enum.EasingStyle.Sine, Enum.EasingDirection.Out),
				(TweenInfo.new(0.2, Enum.EasingStyle.Sine, Enum.EasingDirection.Out))
			g.Field.Focused:Connect(function()
				if j then
					j:Cancel()
				end
				j = f:Create(
					...
					h,
					{
						Color = b.Theme.Controls.SelectionStroke[1].Value,
						Transparency = b.Theme.Controls.SelectionStroke[2].Value,
						Thickness = 3,
					}
				)
				j:Play()
			end)
			g.Field.FocusLost:Connect(function()
				if j then
					j:Cancel()
				end
				j = f:Create(
					...
					i,
					{
						...
						Transparency = b.Theme.Controls.ViewBorder[2].Value,
						Thickness = 1,
					}
				)
				j:Play()
			end)
			g.Field:GetPropertyChangedSignal("Text"):Connect(function()
				if g.Field.Text == "" then
					g.Field.PlaceholderColor3 = b.Theme.Text.Tertiary[1].Value
					g.Field.TextTransparency = b.Theme.Text.Tertiary[2].Value
				else
					g.Field.TextColor3 = b.Theme.Text.Primary[1].Value
					g.Field.TextTransparency = b.Theme.Text.Primary[2].Value
				end
			end)
			return g
		end
	...
	function a.m()
		return function(b)
			local c, d = a.load("d"), a.load("l")(b)
			local e, f, g, h, i, j =
				c.Create, {}, 0, {}, function(e)
					local f, g = e.AbsolutePosition, e.AbsoluteSize
					return { Left = f.X, Right = f.X + g.X, Top = f.Y, Bottom = f.Y + g.Y }
				end
			h.Body = (
				e("Frame")({
					Name = "Toolbar",
					...
					e("UICorner")({ Name = "UICorner", CornerRadius = UDim.new(0, 10) }),
					e("Frame")({
						Name = "Shadow",
						...
						e("UIGradient")({
							...
							__dynamicKeys = {
								Color = b.Theme.Controls.TitlebarShadow.Color,
								Transparency = b.Theme.Controls.TitlebarShadow.Transparency,
							},
						}),
					}),
					e("Frame")({
						...
						e("Frame")({
							...
							e("UIPadding")({ Name = "UIPadding", PaddingLeft = UDim.new(0, 12), PaddingRight = UDim.new(
								0,
								12
							) }),
							e("Frame")({
								...
								e("TextButton")({
									...
									e("ImageLabel")({
										...
										__dynamicKeys = {
											ImageColor3 = b.Theme.Text.Tertiary[1],
											ImageTransparency = b.Theme.Text.Tertiary[2],
										},
										__contextKeys = {
											ImageColor3 = function()
												local k, l = g > 1, j.Theme.Text
												return (k and l.Secondary[1].Value) or l.Tertiary[1].Value
											end,
											ImageTransparency = function()
												local k, l = g > 1, j.Theme.Text
												return (k and l.Secondary[2].Value) or l.Tertiary[2].Value
											end,
										},
									}),
								}),
					...
					e("Frame")({
						...
						ZIndex = -1,
						__dynamicKeys = { BackgroundColor3 = b.Theme.Controls.Titlebar[1] },
					}),
				})
			)
			h.CornerClipLeft = (h.Body:FindFirstChild("CornerClipLeft"))
			h.CornerClipRight = (h.Body:FindFirstChild("ConerClipRight"))
			h.Content = (h.Body:FindFirstChild("Content"))
			h.Leading = (h.Content:FindFirstChild("Leading"))
			h.Trailing = (h.Content:FindFirstChild("Trailing"))
			h.NavigationButtons = (h.Leading:FindFirstChild("NavigationButtons"))
			h.Back = (h.NavigationButtons:FindFirstChild("Back"))
			h.Forward = (h.NavigationButtons:FindFirstChild("Forward"))
			h.SidebarButton = (h.Leading:FindFirstChild("SidebarButton"):FindFirstChild("Image"))
			h.TitleStack = (h.Leading:FindFirstChild("TitleSubtitle"))
			h.Title = (h.TitleStack:FindFirstChild("Title"))
			h.Subtitle = (h.TitleStack:FindFirstChild("Subtitle"))
			h.SearchField = d
			h.SearchField.Body.Size = UDim2.fromOffset(126, 28)
			h.SearchField.Field.Size = UDim2.fromOffset(90, 0)
			h.SearchField.Layout.Padding = UDim.new(0, 8)
			h.SearchField.Field.LayoutOrder = 1
			h.SearchField.Field.PlaceholderText = "Search"
			h.SearchField.Padding.PaddingLeft = UDim.new(0, 7)
			h.SearchField.Padding.PaddingRight = UDim.new(0, 7)
			h.SearchField.Corner = UDim.new(0, 6)
			e("ImageLabel")({
				Name = "ImageLabel",
				BackgroundColor3 = Color3.fromRGB(255, 255, 255),
				BackgroundTransparency = 1,
				...
				Parent = h.SearchField.Body.__instance,
				__dynamicKeys = {
					ImageColor3 = b.Theme.Text.PrimaryAccent[1],
					ImageTransparency = b.Theme.Text.PrimaryAccent[2],
				},
			})
			h.SearchField.Body.Parent = h.Trailing
			local k = function()
				if not j then
					return
				end
				local k, l, m = h.Back:FindFirstChild("Image"), h.Forward:FindFirstChild("Image"), j.Theme.Text
				if k then
					local n = g > 1
					k.ImageColor3 = (n and m.Secondary[1].Value) or m.Tertiary[1].Value
					k.ImageTransparency = (n and m.Secondary[2].Value) or m.Tertiary[2].Value
					h.Back.Interactable = n
				end
				if l then
					local n = g < #f
					l.ImageColor3 = (n and m.Secondary[1].Value) or m.Tertiary[1].Value
					l.ImageTransparency = (n and m.Secondary[2].Value) or m.Tertiary[2].Value
					h.Forward.Interactable = n
				end
			end
			local l, m =
				setmetatable({}, {
					...
					if j and j.Tabs then
						for m, n in pairs(j.Tabs) do
							if n.__container == l then
								n.Selected = true
							else
								n.Selected = false
							end
			...
			local n, o =
				function()
					h.Back.MouseButton1Click:Connect(function()
						if g > 1 then
							g -= 1
							m(f[g])
							k()
					...
					h.Forward.MouseButton1Click:Connect(function()
						if g < #f then
							g += 1
							m(f[g])
							k()
					...
					for n, o in pairs(h.Trailing:GetChildren()) do
						if not o:IsA("GuiObject") then
							continue
						end
						local p, q = i(o), true
						for r, s in pairs(h.Leading:GetChildren()) do
							if not s:IsA("GuiObject") then
								continue
							end
							local t = i(s)
							local u = (t.Right >= p.Left and t.Left <= p.Right) and (t.Bottom >= p.Top and t.Top <= p.Bottom)
							if u then
								q = false
								break
							end
			...
			o()
			h.Body:GetPropertyChangedSignal("AbsoluteSize"):Connect(o)
			return h,
				function(p)
					j = p
					n()
	...
	function a.n()
		return function(b, c)
			...
			c = c or Vector2.new(0, 0)
			f.N = f.Handles:FindFirstChild("N"):FindFirstChild("UIDragDetector")
			f.S = f.Handles:FindFirstChild("S"):FindFirstChild("UIDragDetector")
			f.W = f.Handles:FindFirstChild("W"):FindFirstChild("UIDragDetector")
			f.E = f.Handles:FindFirstChild("E"):FindFirstChild("UIDragDetector")
			f.NE = f.Handles:FindFirstChild("NE"):FindFirstChild("UIDragDetector")
			f.SE = f.Handles:FindFirstChild("SE"):FindFirstChild("UIDragDetector")
			f.NW = f.Handles:FindFirstChild("NW"):FindFirstChild("UIDragDetector")
			f.SW = f.Handles:FindFirstChild("SW"):FindFirstChild("UIDragDetector")
			local g, h, i = {}
			g.__index = g
			local j = function(j, k)
				local l = Vector2.zero
				if string.find(k, "N") then
					l += Vector2.new(0, -1)
				end
				...
				local m = Vector2.new((l.X ~= 0) and (j.X * l.X) or 0, (l.Y ~= 0) and (j.Y * l.Y) or 0)
				local n = i + m
				n = Vector2.new(math.max(c.X, n.X), math.max(c.Y, n.Y))
				local o = n - i
				local p = Vector2.new(l.X < 0 and -o.X * 0.5 or o.X * 0.5, l.Y < 0 and -o.Y * 0.5 or o.Y * 0.5)
				local q = h + p
				b.Size = UDim2.fromOffset(n.X, n.Y)
				b.Position = UDim2.new(0.5, q.X, 0.5, q.Y)
			end
			local k = function(k, l)
				k.DragStart:Connect(function()
					i = b.AbsoluteSize
					h = Vector2.new(b.Position.X.Offset, b.Position.Y.Offset)
				end)
				k.DragContinue:Connect(function()
					j(Vector2.new(k.DragUDim2.X.Offset, k.DragUDim2.Y.Offset), l)
				end)
			end
			function g.CreateClient()
				k(f.NW, "NW")
				k(f.NE, "NE")
				k(f.SW, "SW")
				k(f.SE, "SE")
			...
			function g.SetEnabled(l)
				for m, n in pairs(f) do
					if n:IsA("UIDragDetector") then
						n.Enabled = l
					end
				end
			end
	...
	function a.o()
		a.load("b")
		return function(b, c)
			local d, e, f, g, h, i = a.load("k"), a.load("e"), a.load("d"), a.load("c"), a.load("m")(b)
			local j, k, l, m, n =
				a.load("n"), f.Create, e.UserInputService, e.TweenService, b.__container or b.__instance or b
			...
			c = c or {}
			c.Size = c.Size or UDim2.fromOffset(850, 530)
			c.Maximized = c.Maximized == true
			c.Minimized = c.Minimized == true
			c.Searching = c.Searching ~= false
			c.Resizable = c.Resizable ~= false
			c.Draggable = c.Draggable ~= false
			c.Resizable = c.Resizable ~= false
			c.Dropshadow = c.Dropshadow ~= false
			c.UIBlur = c.UIBlur ~= false
					...
					k("Frame")({
						...
						__contextKeys = {
							BackgroundTransparency = function()
								return c.UIBlur and b.Theme.Controls.Sidebar[2].Value or 0
							end,
						},
						k("UICorner")({ Name = "UICorner", CornerRadius = UDim.new(0, 10) }),
						k("UIListLayout")({
							Name = "UIListLayout",
							FillDirection = Enum.FillDirection.Horizontal,
							...
							k("Frame")({
								...
								k("Folder")({
									...
									k("Frame")({
										...
										ZIndex = 0,
										__dynamicKeys = {
											BackgroundColor3 = b.Theme.Controls.Separator.Shadow[1],
											BackgroundTransparency = b.Theme.Controls.Separator.Shadow[2],
										},
										k("UIGradient")({
											Name = "UIGradient",
											Transparency = NumberSequence.new({
								...
								k("UIPadding")({ Name = "UIPadding", PaddingRight = UDim.new(0, 2) }),
								k("Frame")({
									Name = "Toolbar",
									BackgroundColor3 = Color3.fromRGB(255, 255, 255),
									BackgroundTransparency = 1,
									...
									k("Frame")({
										...
										k("ImageButton")({
											...
											__dynamicKeys = {
												ImageColor3 = b.Theme.Controls.Exit[1],
												ImageTransparency = b.Theme.Controls.Exit[2],
											},
											k("ImageLabel")({
												Name = "Stroke",
												BackgroundColor3 = Color3.fromRGB(255, 255, 255),
											...
											k("ImageLabel")({
												...
												Visible = false,
												__dynamicKeys = {
													ImageColor3 = b.Theme.Controls.WindowControlIcon[1],
													ImageTransparency = b.Theme.Controls.WindowControlIcon[2],
												},
											}),
										}),
										k("ImageButton")({
						...
						k("Frame")({
							...
							__dynamicKeys = {
								BackgroundColor3 = b.Theme.Controls.Background[1],
								BackgroundTransparency = b.Theme.Controls.Background[2],
							},
							k("UICorner")({ Name = "UICorner", CornerRadius = UDim.new(0, 10) }),
							k("Frame")({
								Name = "Content",
			...
			p.Scale = k("UIScale")({ Parent = o })
			p.Cornering = (p.Body:FindFirstChild("UICorner"))
			p.LayoutIgnore = (p.Body:FindFirstChild("LayoutIgnore"))
			p.Dropshadow = (p.LayoutIgnore:FindFirstChild("Dropshadow"))
			p.Separator = (p.Body:FindFirstChild("Separator"))
			p.SidebarMargins = p.Body:FindFirstChild("Sidebar")
			p.Sidebar = (p.SidebarMargins:FindFirstChild("Margins"))
			p.SidebarList = (p.Sidebar:FindFirstChild("SidebarList"))
			p.Toolbar = (p.Sidebar:FindFirstChild("Toolbar"))
			p.WindowControls = (p.Toolbar:FindFirstChild("WindowControls"))
			p.ContentBody = (p.Body:FindFirstChild("ContentBody"))
			p.Content = (p.ContentBody:FindFirstChild("Content"))
			p.CornerClip = (p.ContentBody:FindFirstChild("CornerClip"))
			p.Titlebar = h
			p.Titlebar.Body.Parent = p.ContentBody
			q = p.Body.Size
			r = p.Body.Position
			local w = {
				Title = function(w)
					p.Titlebar.Title.Visible = w and true or false
					if w then
						p.Titlebar.Title.Text = w
					end
				end,
				Subtitle = function(w)
					p.Titlebar.Subtitle.Visible = w and true or false
					if w then
						p.Titlebar.Subtitle.Text = w
					end
				end,
				Searching = function(w)
					p.Titlebar.SearchField.Body.Parent = w and p.Titlebar.Trailing or nil
					p.Titlebar.SearchField.Body.Visible = w
				end,
				Draggable = function(w)
					u = not w and false
				end,
				Resizable = function(w)
					if v and not c.Maximized then
						v.SetEnabled(w)
					end
				end,
				Minimized = function(w, x)
					local y = x ~= false
					if w and not t then
						s = p.Body.Position
						if p.BlurModel then
							p.BlurModel.SetVisibility(false)
						end
					end
					local z, A, B =
						TweenInfo.new(0.25, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out),
						{ Position = w and UDim2.new(0.5, 0, 0.5, p.Body.AbsoluteSize.Y * 2) or s },
						{ Scale = w and 0 or 1 }
					if y then
						t = true
						m:Create(p.Body.__instance, z, A):Play()
						local C = m:Create(p.Scale.__instance, z, B)
						C:Play()
						C.Completed:Once(function()
							t = false
							if p.BlurModel and not c.Minimized then
								p.BlurModel.SetVisibility(true)
							end
						end)
					else
						p.Body.Position = A.Position
						p.Scale.Scale = B.Scale
						if w and p.BlurModel then
							p.BlurModel.SetVisibility(false)
						end
					end
				end,
				Maximized = function(w, x)
					x = x ~= false
					if w then
						q = p.Body.Size
						r = p.Body.Position
					...
					local y, z, A =
						...
						{ Size = w and UDim2.fromScale(1, 1) or q, Position = w and UDim2.fromScale(0.5, 0.5) or r },
						{ CornerRadius = w and UDim.new(0, 0) or UDim.new(0, 10) }
					if x then
						m:Create(p.Body.__instance, y, z):Play()
						m:Create(p.Cornering, y, A):Play()
					else
						p.Body.Size = z.Size
						p.Body.Position = z.Position
						p.Cornering.CornerRadius = A.CornerRadius
					end
					if not w then
						v.SetEnabled(c.Resizable)
					end
				end,
				Dropshadow = function(w)
					p.Dropshadow.Visible = w
				end,
				UIBlur = function(w)
					if p.BlurModel then
						p.BlurModel.Model:Destroy()
						p.BlurModel = nil
					end
					p.Body.BackgroundTransparency = w and b.Theme.Controls.Sidebar[2].Value or 0
					if w and not c.Minimized then
						p.BlurModel = d.UIBlur(p.Body)
					end
				end,
			}
			local x = g.Wrap(c, w, p.Body)
			x.Type = "Window"
			x.Theme = b.Theme
			x.Structures = p
			x.Tabs = {}
			...
			do
				v = j(p.Body.__instance, Vector2.new(350, 250))
				v.CreateClient()
			end
			do
				local y = true
				local z = function()
					y = not y
					local z, A, B, C, D =
						y and UDim.new(0, 0) or UDim.new(0, -200),
						y and UDim2.new(0, 200, 1, 0) or UDim2.new(0, 0, 1, 0),
						y and UDim2.new(1, -201, 1, 0) or UDim2.new(1, 0, 1, 0),
						y and UDim2.new(0, 10, 1, 0) or UDim2.new(0, 10, 0.5, 0),
						TweenInfo.new(0.6, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out)
					p.Separator.Visible = y and true or false
					p.CornerClip.Visible = y and true
					m:Create(p.ContentBody, D, { Size = B }):Play()
					m:Create(p.SidebarMargins:FindFirstChild("UIPadding"), D, { PaddingLeft = z }):Play()
					local E = m:Create(p.SidebarMargins, D, { Size = A })
					m:Create(p.Titlebar.CornerClipLeft, D, { Size = C }):Play()
					E:Play()
					if not y then
				...
				p.Titlebar.SidebarButton.MouseButton1Click:Connect(z)
			end
			do
				local y, z = p.Titlebar.SearchField.Field, {}
				local A, B =
					function(A, B)
						local C = {}
				...
				y:GetPropertyChangedSignal("Text"):Connect(function()
					local C, D, E = {}, {}, p.Content:FindFirstChildOfClass("ScrollingFrame")
					if not E then
						return
					end
					local F = E:GetDescendants()
					if not z[E] then
						A(E, F)
					end
					local G = y.Text:lower()
					if G == "" then
						B(E, F)
						return
					end
					for H, I in ipairs(F) do
						if I:IsA("GuiObject") then
							local J = I:GetAttribute("SearchIndex")
							if typeof(J) == "string" and J:lower():find(G) then
								table.insert(C, I)
							end
						end
					end
					...
					for H, I in ipairs(C) do
						...
						local K = z[E]
						for L, M in ipairs(I:GetDescendants()) do
			...
			do
				local y, z, A =
					...
					(p.WindowControls:FindFirstChild("Minimize")),
					(p.WindowControls:FindFirstChild("Zoom"))
				local B = function()
					y:FindFirstChild("Icon").Visible = p.WindowControls.GuiState ~= Enum.GuiState.Hover
					z:FindFirstChild("Icon").Visible = p.WindowControls.GuiState ~= Enum.GuiState.Hover
					A:FindFirstChild("Icon").Visible = p.WindowControls.GuiState ~= Enum.GuiState.Hover
				end
				A.MouseButton1Click:Connect(function()
					c.Maximized = not c.Maximized
					w.Maximized(c.Maximized, true)
				end)
				z.MouseButton1Click:Connect(function()
					c.Minimized = not c.Minimized
					w.Minimized(c.Minimized, true)
				end)
				if b.Structures and b.Structures.WindowPill then
					local C = b.Structures.WindowPill
				...
				y.MouseButton1Click:Connect(function()
					w.Minimized(true)
					task.delay(0.25, p.Body.Destroy)
				end)
				p.WindowControls.MouseEnter:Connect(B)
				p.WindowControls.MouseLeave:Connect(B)
			end
			do
				local y, z, A, B = (p.LayoutIgnore:FindFirstChild("TopArea"))
				y.InputBegan:Connect(function(C)
					if
						(
				...
				l.InputChanged:Connect(function(C)
					if
						C.UserInputType == Enum.UserInputType.MouseMovement
						or C.UserInputType == Enum.UserInputType.Touch
					then
						if u then
							local D = C.Position - z
							local E = UDim2.new(A.X.Scale, A.X.Offset + D.X, A.Y.Scale, A.Y.Offset + D.Y)
							p.Body.Position = E
						end
					end
				end)
			...
			g.Apply(c, x)
			local y = function()
				for y, z in pairs(p.Body:GetDescendants()) do
	...
	function a.p()
		a.load("b")
		return function(b, c)
			local d, e, f = a.load("d"), a.load("c"), a.load("e")
			local g, h, i, j, k =
				d.Create,
				f.TweenService,
				b.Type == "Window" and b.Structures.SidebarList or b.__instance or b,
				{},
				function(g, h, i, ...)
					local j = table.pack(...)
					for k, l in ipairs(g:GetChildren()) do
						if l:IsA("GuiObject") then
							table.insert(
								h,
								l:GetPropertyChangedSignal("AbsoluteSize"):Connect(function(...)
									i(table.unpack(j, 1, j.n))
								end)
							)
							table.insert(
								h,
					...
					table.insert(
						h,
						g.ChildAdded:Connect(function(k)
					...
					table.insert(
						h,
						g.ChildRemoved:Connect(function(...)
							i(table.unpack(j, 1, j.n))
						end)
					)
				end
			c = c or {}
			c.Title = c.Title or "Section Title"
			c.Disclosure = c.Disclosure ~= false
			...
			j.Layout = (j.SidebarGroup:FindFirstChild("UIListLayout"))
			j.SectionHeader = (j.SidebarGroup:FindFirstChild("SectionHeader"))
			j.Title = (j.SectionHeader:FindFirstChild("Title"))
			j.Disclosure = (j.SectionHeader:FindFirstChild("Disclosure"))
			j.DisclosureImage = (j.Disclosure:FindFirstChild("DisclosureImage"))
			local l = {}
			local m = {
				Title = function(m)
					j.Title.Text = m
				end,
				Disclosure = function(m)
					j.Disclosure.Visible = m
					j.SidebarGroup.Active = m
				end,
				Expanded = function(m, n)
					n = (n == nil and true) or false
					local o, p = j.SidebarGroup.__instance, j.DisclosureImage
					...
					local q = function(q)
						local r = m and j.Layout.AbsoluteContentSize.Y or 23
						local s = h:Create(
							o,
							TweenInfo.new(q and 0.35 or 0, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out),
					...
					h:Create(
						p,
						TweenInfo.new(n and 0.35 or 0, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out),
						{ Rotation = m and 0 or -90 }
					):Play()
				end,
			}
			local n = e.Wrap(c, m, j.SidebarGroup)
			n.Type = "Section"
			n.Theme = b.Theme
			n.Structures = j
			n.__window = b
			j.SidebarGroup.MouseButton1Click:Connect(function()
				if not c.Disclosure then
					return
				end
	...
	function a.q()
		...
		return function(b, c)
			...
			local f, g, h, i =
				d.Create,
				b.Type == "Window" and b or b.__window,
				b.Type == "Window" and b.Structures.SidebarList
					or b.Type == "Tab" and b.__instance.Parent
					or b.__instance
					or b,
				{}
			c = c or {}
			c.Title = c.Title or "Label"
			c.Indentation = c.Indentation or 0
			...
			i.Body = (
				e.Apply(
					...
					f("TextButton")({
						...
						BorderSizePixel = 0,
						FontFace = Font.new("rbxasset://fonts/families/SourceSansPro.json"),
						LayoutOrder = g.Tabs and #g.Tabs or 1,
						...
						__contextKeys = {
							BackgroundColor3 = function()
								return c.Selected and b.Theme.Controls.SelectionFocused[1].Value or 1
							end,
							BackgroundTransparency = function()
								return c.Selected and b.Theme.Controls.SelectionFocused[2].Value or 1
							end,
						},
						f("UICorner")({
							Name = "UICorner",
						...
						f("UIPadding")({ Name = "UIPadding", PaddingLeft = UDim.new(0, 10), PaddingRight = UDim.new(
							0,
							8
						) }),
						f("Frame")({
							...
							f("TextLabel")({
								...
								TextXAlignment = Enum.TextXAlignment.Left,
								__dynamicKeys = {
									TextColor3 = b.Theme.Text.SelectionPrimary[1],
									TextTransparency = b.Theme.Text.SelectionPrimary[2],
								},
								__contextKeys = {
									TextColor3 = function()
										return c.Selected and b.Theme.Text.SelectionPrimary[1].Value
											or b.Theme.Text.Primary[1].Value
									end,
									TextTransparency = function()
										return c.Selected and b.Theme.Text.SelectionPrimary[2].Value
											or b.Theme.Text.Primary[2].Value
									end,
								},
							}),
							...
							f("ImageLabel")({
								...
								__contextKeys = {
									ImageColor3 = function()
										return c.Selected and b.Theme.Text.SelectionPrimary[1].Value
											or b.Theme.Controls.Selection[1].Value
									end,
									ImageTransparency = function()
										return c.Selected and b.Theme.Text.SelectionPrimary[2].Value
											or b.Theme.Controls.Selection[2].Value
									end,
								},
							}),
						}),
			...
			i.Trailing = (i.Body:FindFirstChild("Trailing"))
			i.Leading = (i.Body:FindFirstChild("Leading"))
			i.Title = (i.Leading:FindFirstChild("Title"))
			i.Symbol = (i.Leading:FindFirstChild("Symbol"))
			i.Padding = (i.Body:FindFirstChild("UIPadding"))
			i.Page = (
				f("ScrollingFrame")({
					Name = "ContentList",
					AutomaticCanvasSize = Enum.AutomaticSize.Y,
			...
			local j = e.Wrap(c, {
				...
				Icon = function(j)
					i.Symbol.Visible = j and true or false
					if j then
						i.Symbol.Image = j
					end
				end,
				Indentation = function(j)
					i.Padding.PaddingLeft += UDim.new(0, (11 * j))
				end,
				Selected = function(j)
					i.Body.Transparency = j and b.Theme.Controls.SelectionFocused[2].Value or 1
					i.Title.TextColor3 = j and b.Theme.Text.SelectionPrimary[1].Value or b.Theme.Text.Primary[1].Value
					i.Title.TextTransparency = j and b.Theme.Text.SelectionPrimary[2].Value
						or b.Theme.Text.Primary[2].Value
					i.Symbol.ImageColor3 = j and b.Theme.Text.SelectionPrimary[1].Value
						or b.Theme.Controls.Selection[1].Value
					i.Symbol.ImageTransparency = j and b.Theme.Text.SelectionPrimary[2].Value
						or b.Theme.Controls.Selection[2].Value
					i.Page.Parent = j and g and g.__container or nil
				end,
			}, i.Body)
			j.Type = "Tab"
			j.Theme = b.Theme
			j.Structures = i
			j.__container = i.Page.__instance
			j.__window = b.__window
			e.Apply(c, j)
			if g then
				table.insert(g.Tabs, j)
				if j.Selected then
					g.addToHistory(j.__container)
				end
				i.Body.MouseButton1Click:Connect(function()
					if j.Selected then
						return
					end
					for k, l in pairs(g.Tabs) do
						l.Selected = false
					end
					j.Selected = true
					g.addToHistory(j.__container)
	...
	function a.r()
		return function(b)
			...
			e.Title = (e.Body:FindFirstChild("Title"))
			e.Subtitle = (e.Body:FindFirstChild("Subtitle"))
			e.Padding = (e.Body:FindFirstChild("UIPadding"))
			return e
		end
	end
	function a.s()
		a.load("b")
		return function(b, c)
			local d, e, f = a.load("d"), a.load("c"), a.load("r")
			local g, h, i = d.Create, b.__container or b.__instance or b, f(b)
			c = c or {}
			c.Title = c.Title or "Title"
			local j, k = h:FindFirstChild("PageSections"), true
			if not j then
				k = false
				j = g("Frame")({
					Name = "PageSections",
			...
			local l = g("Frame")({
				...
				Size = UDim2.fromScale(1, 0),
				AutomaticSize = Enum.AutomaticSize.Y,
				Parent = k and j or j.__instance,
				g("UIListLayout")({ Name = "UIListLayout", Padding = UDim.new(0, 12), SortOrder = Enum.SortOrder.LayoutOrder }),
			})
			i.Body.Parent = l.__instance
			i.Padding.PaddingLeft = UDim.new(0, 8)
			i.Padding.PaddingRight = UDim.new(0, 8)
			i.Title.FontFace = Font.new(i.Title.FontFace.Family, Enum.FontWeight.SemiBold)
			i.Body.LayoutOrder = -1
			local m = e.Wrap(c, {
				Title = function(m)
					i.Title.Text = m
				end,
	...
	function a.t()
		...
		return function(b, c)
			local d, e = a.load("d"), a.load("c")
			local f, g, h, i, j = d.Create, b.__container or b.__instance or b, {}, {}, {}
			c = c or {}
			h.Body = (
				e.Apply(
					c,
					f("Frame")({
						...
						Parent = g,
						__dynamicKeys = {
							BackgroundColor3 = b.Theme.Controls.View[1],
							BackgroundTransparency = b.Theme.Controls.View[2],
						},
						f("UIListLayout")({ Name = "UIListLayout", SortOrder = Enum.SortOrder.LayoutOrder }),
						f("UIPadding")({ Name = "Margins", PaddingLeft = UDim.new(0, 10), PaddingRight = UDim.new(
							0,
							10
						) }),
						f("UICorner")({ Name = "UICorner", CornerRadius = UDim.new(0, 6) }),
						f("UIStroke")({
							Name = "UIStroke",
							ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
							__dynamicKeys = {
			...
			local k = e.Wrap(c, {}, h.Body)
			k.Type = "Form"
			k.Theme = b.Theme
			k.Structures = h
			local l = function()
				for l, m in ipairs(j) do
					local n, o = m.section.Visible, j[l + 1]
					local p = o and o.section.Visible
					m.divider.Visible = n and p == true
				end
			end
			h.Body.ChildAdded:Connect(function(m)
				if m:IsA("Frame") and m:FindFirstChild("LayoutIgnore") then
					local n = m.LayoutIgnore:FindFirstChild("Divider")
					if n then
						table.insert(i, n)
						table.insert(j, { section = m, divider = n })
	...
	function a.u()
		...
		return function(b, c)
			local d, e = a.load("d"), a.load("c")
			local f, g, h = d.Create, b.__container or b.__instance or b, {}
			c = c or {}
			h.Body = (
				e.Apply(
					c,
			...
			h.LeftAccessories = (h.Body:FindFirstChild("LeftAccessory"))
			h.RightAccessories = (h.Body:FindFirstChild("RightAccessory"))
			local i = function()
				local i, j, k = h.LeftAccessories.Parent.AbsoluteSize.X, h.RightAccessories.AbsoluteSize.X, 75
				local l = math.max(i - j, k)
				if i - j < k then
					h.LeftAccessories.Size = UDim2.new(0, k, 1, 0)
				else
					h.LeftAccessories.Size = UDim2.new(0, l, 1, 0)
				end
			end
			i()
			h.RightAccessories:GetPropertyChangedSignal("AbsoluteSize"):Connect(i)
			h.LeftAccessories.Parent:GetPropertyChangedSignal("AbsoluteSize"):Connect(i)
			local j = e.Wrap(c, {
				SearchIndex = function(j)
					h.Body:SetAttribute("SearchIndex", j)
				end,
			}, h.Body)
			j.Type = "Row"
			j.Theme = b.Theme
			...
			j.Left = function()
				...
				k.__container = h.LeftAccessories
				return k
			end
			j.Right = function()
				local k = table.clone(j)
				k.__container = h.RightAccessories
				return k
			end
			e.Apply(c, j)
			return j
	...
	function a.v()
		...
		return function(b, c)
			...
			local f, g, h = d.Create, b.__container or b.__instance or b, {}
			c = c or {}
			c.Padding = c.Padding or UDim.new(0, 10)
			c.VerticalAlignment = c.VerticalAlignment or Enum.VerticalAlignment.Center
			c.HorizontalAlignment = c.HorizontalAlignment or Enum.HorizontalAlignment.Right
			h.Body = (
				e.Apply(
					c,
					f("Frame")({
			...
			h.Layout = (h.Body:FindFirstChild("UIListLayout"))
			local i = e.Wrap(c, {
				Padding = function(i)
					h.Layout.Padding = i
				end,
				HorizontalAlignment = function(i)
					h.Layout.HorizontalAlignment = i
				end,
				VerticalAlignment = function(i)
	...
	function a.x()
		a.load("b")
		return function(b, c)
			local d, e = a.load("c"), a.load("r")
			local f = e(b)
			c = c or {}
			c.Title = c.Title or "Title"
			f.Body.Parent = b.__container or b.__instance or b
			local g = d.Wrap(c, {
				Title = function(g)
					f.Title.Text = g
				end,
				Subtitle = function(g)
					f.Subtitle.Visible = true
					f.Subtitle.Text = g
				end,
			}, f.Body)
			g.Type = "TitleStack"
	...
	function a.z()
		...
		return function(b, c)
			...
			local f, g, h = d.Create, b.__container or b.__instance or b, {}
			c = c or {}
			c.Style = c.Style or "Primary"
			c.Size = c.Size or UDim2.fromOffset(20, 20)
			h.Body = (
				f("ImageLabel")({
					Name = "Image",
					AutomaticSize = Enum.AutomaticSize.XY,
					...
					BorderSizePixel = 0,
					Parent = g,
					__dynamicKeys = { ImageColor3 = b.Theme.Text.Primary[1], ImageTransparency = b.Theme.Text.Primary[2] },
					__contextKeys = {
						ImageColor3 = function()
							return c.Style == "Primary" and b.Theme.Text.Primary[1].Value
								or b.Theme.Text.Secondary[1].Value
						end,
						ImageTransparency = function()
							return c.Style == "Primary" and b.Theme.Text.Primary[2].Value
								or b.Theme.Text.Secondary[2].Value
						end,
					},
				})
			)
			local i = e.Wrap(c, {
				Style = function(i)
					if i == "Primary" then
						h.Body.ImageColor3 = b.Theme.Text.Primary[1].Value.Value
						h.Body.ImageTransparency = b.Theme.Text.Primary[2].Value
					else
						h.Body.ImageColor3 = b.Theme.Text.Secondary[1].Value
						h.Body.ImageTransparency = b.Theme.Text.Secondary[2].Value
					end
				end,
			}, h.Body)
			i.Type = "Symbol"
	...
	function a.A()
		...
		return function(b, c)
			local d, e, f = a.load("d"), a.load("c"), a.load("e")
			local g, h, i, j, k =
				d.Create, f.TweenService, b.__container or b.__instance or b, b.Theme.Controls.Toggle, {}
			c = c or {}
				...
				e.Apply(
					...
					g("Frame")({
						...
						g("ImageButton")({
							...
							Size = UDim2.fromOffset(28, 16),
							__dynamicKeys = { ImageColor3 = j.SwitchOff[1], ImageTransparency = j.SwitchOff[2] },
							__contextKeys = {
								ImageColor3 = function()
									return c.Value and j.SwitchOn[1].Value or j.SwitchOff[1].Value
								end,
								ImageTransparency = function()
									return c.Value and j.SwitchOn[2].Value or j.SwitchOff[2].Value
								end,
							},
							g("ImageLabel")({
								Name = "Knob",
								...
								__dynamicKeys = { ImageColor3 = j.Knob[1], ImageTransparency = j.Knob[2] },
								g("ImageLabel")({
									Name = "Effects",
									AnchorPoint = Vector2.new(0.5, 0.5),
									BackgroundColor3 = Color3.fromRGB(255, 255, 255),
									...
									__dynamicKeys = {
										ImageColor3 = j.KnobEffects[1],
										ImageTransparency = j.KnobEffects[2],
									},
								}),
							}),
							g("UIGradient")({ Name = "UIGradient", Rotation = 90, __dynamicKeys = {
			...
			k.Switch = (k.Body:FindFirstChild("Switch"))
			k.Knob = (k.Switch:FindFirstChild("Knob"))
			local l
			local m = {
				Value = function(m, n)
					n = (n == nil and true) or false
					local o, p, q =
						...
						{ Position = m and UDim2.new(0, 13, 0.5, 0) or UDim2.new(0, 1, 0.5, 0) },
						{
					...
					if n then
						h:Create(k.Knob, o, p):Play()
						h:Create(k.Switch, o, q):Play()
					else
						k.Knob.Position = p.Position
						k.Switch.ImageColor3 = q.ImageColor3
						k.Switch.ImageTransparency = q.ImageTransparency
					end
					if c.ValueChanged and n then
						c.Value = m
						task.spawn(c.ValueChanged, l, m)
					end
				end,
			}
			l = e.Wrap(c, m, k.Body)
			...
			l.Structures = k
			do
				k.Switch.MouseButton1Click:Connect(function()
					l.Value = not l.Value
				end)
			end
			m.Value(not not c.Value, false)
			e.Apply(c, l)
			return l
		end
	end
	function a.B()
		a.load("b")
		return function(b, c)
			local d, e = a.load("c"), a.load("l")(b)
			c = c or {}
			c.Value = c.Value ~= nil and c.Value or ""
			c.Placeholder = c.Placeholder or "Placeholder"
			e.Field.TextTruncate = Enum.TextTruncate.AtEnd
			e.Field.TextXAlignment = Enum.TextXAlignment.Right
			e.Body.Parent = b.__container or b.__instance or b
			local f
			f = d.Wrap(c, {
				...
				Value = function(g)
					...
					if c.ValueChanged then
						c.Value = g
						task.spawn(c.ValueChanged, f, g)
					end
				end,
			}, e.Body)
			e.Field.Focused:Connect(function()
				e.Body.AutomaticSize = Enum.AutomaticSize.XY
			end)
			e.Field.FocusLost:Connect(function()
				e.Body.AutomaticSize = Enum.AutomaticSize.None
				if f.Value ~= e.Field.Text then
					f.Value = e.Field.Text
				end
			end)
			f.Type = "TextField"
			f.Theme = b.Theme
			f.Structures = e
			d.Apply(c, f)
			e.Field:GetPropertyChangedSignal("Text"):Connect(function()
				if c.TextChanged then
					task.spawn(c.TextChanged, f, e.Field.Text or "")
				end
			end)
			return f
		end
	...
	function a.C()
		a.load("b")
		return function(b, c)
			local d, e = a.load("c"), a.load("e")
			local f, g = e.UserInputService, a.load("l")(b)
			c = c or {}
			c.Placeholder = c.Placeholder or "Press a key"
			g.Field.TextXAlignment = Enum.TextXAlignment.Right
			g.Field.TextEditable = false
			g.Body.Parent = b.__container or b.__instance or b
			g.Body.AutomaticSize = Enum.AutomaticSize.XY
			g.Body.Size = UDim2.fromOffset(0, 23)
			g.Field.Size = UDim2.fromOffset(0, 23)
			g.Corner.CornerRadius = UDim.new(0, 6)
			local h
			h = d.Wrap(c, {
				...
				Value = function(i)
					...
					if c.ValueChanged then
						c.Value = i
						task.spawn(c.ValueChanged, h, i)
					end
				end,
			}, g.Body)
			f.InputBegan:Connect(function(i, j)
				if i.UserInputType ~= Enum.UserInputType.Keyboard then
					return
				end
				if i.KeyCode == c.Value and c.BindPressed then
					c.BindPressed(h, i.KeyCode, false, j)
				end
			end)
			f.InputEnded:Connect(function(i, j)
				if i.UserInputType ~= Enum.UserInputType.Keyboard then
					return
				end
				if g.Field:IsFocused() then
					h.Value = i.KeyCode
					g.Field:ReleaseFocus()
				elseif i.KeyCode == c.Value and c.BindPressed then
					c.BindPressed(h, i.KeyCode, true, j)
				end
			end)
			h.Type = "KeybindField"
			h.Theme = b.Theme
	...
	function a.D()
		...
		return function(b, c)
			local d, e = a.load("d"), a.load("c")
			local f, g, h, i, j = d.Create, b.__container or b.__instance or b, b.Theme.Controls.Slider, 0, {}
			c = c or {}
			c.Value = c.Value or 0
			c.Maximum = c.Maximum or 1
			c.Minimum = c.Minimum or 0
			j.Body = (
				e.Apply(
					c,
					f("ImageLabel")({
						...
						Parent = g,
						__dynamicKeys = { ImageColor3 = h.Track[1], ImageTransparency = h.Track[2] },
						f("ImageLabel")({
							Name = "TrackClip",
							BackgroundColor3 = Color3.fromRGB(255, 255, 255),
							BackgroundTransparency = 1,
							...
							ZIndex = 3,
							__dynamicKeys = {
								ImageColor3 = b.Theme.Controls.View[1],
								ImageTransparency = b.Theme.Controls.View[2],
							},
						}),
						f("Frame")({
							Name = "Fill",
							...
							f("ImageLabel")({
								...
								ZIndex = 0,
								__dynamicKeys = { ImageColor3 = h.TrackEffects[1], ImageTransparency = h.TrackEffects[2] },
							}),
							f("Frame")({
								Name = "Thumb",
								Active = true,
								...
								f("ImageLabel")({
									...
									Selectable = true,
									Size = UDim2.fromOffset(20, 20),
									__dynamicKeys = { ImageColor3 = h.Thumb[1], ImageTransparency = h.Thumb[2] },
									f("UIStroke")({
										Name = "UIStroke",
										ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
										__dynamicKeys = { Color = h.ThumbStroke[1], Transparency = h.ThumbStroke[2] },
									}),
									f("UICorner")({ Name = "UICorner", CornerRadius = UDim.new(1, 0) }),
								}),
								f("ImageLabel")({
									Name = "ThumbEffects",
									AnchorPoint = Vector2.new(0.5, 0),
									...
									ZIndex = 0,
									__dynamicKeys = {
										ImageColor3 = h.ThumbEffects[1],
										ImageTransparency = h.ThumbEffects[2],
									},
									f("UICorner")({ Name = "UICorner", CornerRadius = UDim.new(1, 0) }),
								}),
							}),
			...
			j.Fill = (j.Body:FindFirstChild("Fill"))
			j.Thumb = (j.Fill:FindFirstChild("Thumb"))
			j.Dragger = (
				f("UIDragDetector")({
					Name = "UIDragDetector",
					ResponseStyle = Enum.UIDragDetectorResponseStyle.CustomOffset,
			...
			local l = {
				Value = function(l)
					local m, n = j.Body.AbsoluteSize.X, j.Thumb.AbsoluteSize.X
					local o, p, q = n / 2, c.Minimum, c.Maximum
					local r, s = (l - p) / (q - p), m - n
					local t = o + (s * r)
					j.Fill.Size = UDim2.new(0, t, 1, 0)
					if c.ValueChanged then
						c.Value = l
			...
			k.Theme = b.Theme
			k.Structures = j
			j.Dragger.DragStart:Connect(function()
				i = j.Fill.AbsoluteSize.X
			end)
			j.Dragger.DragContinue:Connect(function()
				local m, n = j.Body.AbsoluteSize.X, j.Thumb.AbsoluteSize.X
				local o = n / 2
				local p, q, r = o, m - o, j.Dragger.DragUDim2.X.Offset
				local s = i + r
				local t = math.clamp(s, p, q)
				local u = (t - p) / (q - p)
				k.Value = k.Minimum + (k.Maximum - k.Minimum) * u
			end)
			e.Apply(c, k)
			return k
		end
	...
	function a.E()
		...
		return function(b, c)
			local d, e, f = a.load("d"), a.load("c"), a.load("e")
			local g, h, i, j, k =
				d.Create, f.TweenService, b.__container or b.__instance or b, b.Theme.Controls.Button, {}
			c = c or {}
			c.State = c.State or "Primary"
			...
			k.Body = (
				e.Apply(
					...
					g("TextButton")({
						...
						Text = "",
						Parent = i,
						g("UICorner")({ Name = "UICorner", CornerRadius = UDim.new(0, 5) }),
						g("Folder")({
							Name = "ShadowLayers",
							g("Frame")({
								Name = "Layer1",
			...
			k.Shadows = k.Body:FindFirstChild("ShadowLayers")
			k.Shadow1 = k.Shadows:FindFirstChild("Layer1"):FindFirstChild("UIStroke")
			k.Shadow2 = k.Shadows:FindFirstChild("Layer2"):FindFirstChild("UIStroke")
			local l, m =
				{
					TextColor3 = function()
						local l = c.State
						return l == "Primary" and b.Theme.Text.SelectionPrimary[1].Value or l == "Secondary" and b.Theme.Text.Primary[1].Value or l == "Destructive" and b.Theme.Accents.Red[1].Value
					end,
					TextTransparency = function()
						local l = c.State
						return l == "Primary" and b.Theme.Text.SelectionPrimary[2].Value or l == "Secondary" and b.Theme.Text.Primary[2].Value or l == "Destructive" and b.Theme.Accents.Red[2].Value
					end,
				}, {
					Color = function()
						local l = c.State
						return l == "Primary" and j.FillPrimary.Value or (l == "Secondary" or l == "Destructive") and j.FillSecondary.Value
					end,
				}
			k.Label = (
				g("TextLabel")({
			...
			k.Fill = (
				g("UIGradient")({
					...
					Rotation = 90,
					Parent = k.Body.__instance,
					__dynamicKeys = { Color = j.FillPrimary },
					__contextKeys = m,
				})
			)
			local n, o = {
			...
			o.Theme = b.Theme
			o.Structures = k
			k.Body.MouseButton1Click:Connect(function()
				if c.Pushed then
					task.spawn(c.Pushed, o)
				end
			end)
			k.Body.MouseButton1Down:Connect(function()
				local p = TweenInfo.new(0.08, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out)
				h:Create(k.Shadow1, p, { Transparency = 0.8 }):Play()
				h:Create(k.Shadow2, p, { Transparency = 0.9 }):Play()
			end)
			k.Body.MouseButton1Up:Connect(function()
				local p = TweenInfo.new(0.12, Enum.EasingStyle.Sine, Enum.EasingDirection.Out)
				h:Create(k.Shadow1, p, {
					Transparency = 0.95,
				}):Play()
				h:Create(k.Shadow2, p, { Transparency = 0.98 }):Play()
			end)
			k.Body.MouseLeave:Connect(function()
				local p = TweenInfo.new(0.12, Enum.EasingStyle.Sine, Enum.EasingDirection.Out)
				h:Create(k.Shadow1, p, { Transparency = 0.95 }):Play()
				h:Create(k.Shadow2, p, { Transparency = 0.98 }):Play()
			end)
			e.Apply(c, o)
			return o
	...
	function a.F()
		a.load("b")
		return function(b, c)
			local d, e, f = a.load("d"), a.load("c"), a.load("l")
			local g, h, i, j, k, l, m, n, o =
				d.Create,
				b.__container or b.__instance or b,
				b.Theme.Controls.Stepper,
				{},
				false,
				{},
				0.35,
			...
			c = c or {}
			c.Value = c.Value or 0
			c.Step = c.Step or 0.1
			c.Maximum = c.Maximum or 1
			c.Minimum = c.Minimum or 0
			c.Fielded = c.Fielded or false
			j.Body = (
				e.Apply(
					c,
					g("ImageLabel")({
						...
						Parent = h,
						__dynamicKeys = { ImageColor3 = i.Background[1], ImageTransparency = i.Background[2] },
						g("ImageLabel")({
							Name = "Shadow",
							BackgroundTransparency = 1,
							AnchorPoint = Vector2.new(0.5, 0.5),
							...
							__dynamicKeys = { ImageColor3 = i.Dropshadow[1], ImageTransparency = i.Dropshadow[2] },
						}),
						g("ImageButton")({
							Name = "Up",
							AutoButtonColor = false,
							...
							__contextKeys = {
								ImageColor3 = function()
									if c.Value >= c.Maximum then
										return b.Theme.Text.Tertiary[1].Value
									else
										return b.Theme.Text.Primary[1].Value
									end
								end,
								ImageTransparency = function()
									if c.Value >= c.Maximum then
										return b.Theme.Text.Tertiary[2].Value
									else
										return b.Theme.Text.Primary[2].Value
									end
								end,
							},
						}),
						...
						g("Frame")({
							...
							g("UIGradient")({
								...
								__dynamicKeys = { Color = i.SegmentShadow },
							}),
						}),
						g("Frame")({
							Name = "Separator",
							...
							Size = UDim2.new(1, 0, 0, 1),
							ZIndex = 0,
							__dynamicKeys = { BackgroundColor3 = i.Separator[1], BackgroundTransparency = i.Separator[2] },
						}),
						g("Frame")({
							Name = "Bottom",
							BackgroundColor3 = Color3.fromRGB(255, 255, 255),
						...
						g("Frame")({
							...
							ZIndex = 0,
							__dynamicKeys = {
								BackgroundColor3 = i.Filler[1],
								BackgroundTransparency = i.Filler[2],
							},
						}),
					})
				)
			)
			j.IPlus = (j.Body:FindFirstChild("Up"))
			j.IMinus = (j.Body:FindFirstChild("Down"))
			j.Field = f(b)
			j.Field.Padding.PaddingBottom = UDim.new(0, 2)
			j.Field.Padding.PaddingTop = UDim.new(0, 2)
			j.Field.Padding.PaddingRight = UDim.new(0, 2)
			j.Field.Body.Size = UDim2.fromOffset(0, 23)
			j.Field.Field.Size = UDim2.fromOffset(0, 0)
			j.Field.Corner.CornerRadius = UDim.new(0, 6)
			j.Field.FieldPadding:Destroy()
			local r
			local s = {
				Fielded = function(s)
					if s then
						j.Field.Body.Parent = h
						j.Body.Parent = j.Field.Body.__instance
						j.Body.LayoutOrder = 1
					else
						j.Field.Body.Parent = nil
						j.Body.Parent = h
						j.Body.LayoutOrder = 0
					end
				end,
				Value = function(s)
					if s <= c.Minimum then
						j.IMinus.ImageColor3 = b.Theme.Text.Tertiary[1].Value
						j.IMinus.ImageTransparency = b.Theme.Text.Tertiary[2].Value
						j.IMinus.Interactable = false
					else
						j.IMinus.ImageColor3 = b.Theme.Text.Primary[1].Value
						j.IMinus.ImageTransparency = b.Theme.Text.Primary[2].Value
						j.IMinus.Interactable = true
					end
					if s >= c.Maximum then
						j.IPlus.ImageColor3 = b.Theme.Text.Tertiary[1].Value
						j.IPlus.ImageTransparency = b.Theme.Text.Tertiary[2].Value
						j.IPlus.Interactable = false
					else
						j.IPlus.ImageColor3 = b.Theme.Text.Primary[1].Value
						j.IPlus.ImageTransparency = b.Theme.Text.Primary[2].Value
						j.IPlus.Interactable = true
					end
					if j.Field then
						j.Field.Field.Text = p(s, c.Step)
					end
					if c.ValueChanged then
						c.Value = s
						task.spawn(c.ValueChanged, r, s)
					end
				end,
			}
			r = e.Wrap(c, s, j.Body)
			...
			r.Increment = function()
				assert(c.Minimum)
				assert(c.Maximum)
				local t, u = math.max(o(r.Value), o(c.Step)), r.Value + c.Step
				local v = q(u, t)
				r.Value = math.clamp(v, c.Minimum, c.Maximum)
			end
			r.Decrement = function()
				assert(c.Minimum)
				assert(c.Maximum)
				local t, u = math.max(o(r.Value), o(c.Step)), r.Value - c.Step
				local v = q(u, t)
			...
			local t, u =
				function(t)
					k = true
					local u = (t == "Up") and r.Increment or r.Decrement
					l[t] = task.spawn(function()
						if k then
							task.wait(m)
						end
						if not k then
			...
			j.IPlus.MouseButton1Down:Connect(function()
				r.Increment()
				t("Up")
			end)
			j.IPlus.MouseButton1Up:Connect(function()
				u("Up")
			end)
			j.IPlus.MouseLeave:Connect(function()
				u("Up")
			end)
			j.IMinus.MouseButton1Down:Connect(function()
				r.Decrement()
				t("Down")
			end)
			j.IMinus.MouseButton1Up:Connect(function()
				u("Down")
			end)
			j.IMinus.MouseLeave:Connect(function()
				u("Down")
			end)
			j.Body.AncestryChanged:Connect(function(v, w)
				if not w then
					u("Up")
					u("Down")
			...
			j.Field.Field.FocusLost:Connect(function(v)
				if v then
					assert(c.Minimum)
					assert(c.Maximum)
					local w = j.Field.Field.Text
					local x = tonumber(w)
					if x and c.Fielded then
						c.Value = math.clamp(x, c.Minimum, c.Maximum)
						if c.Value then
							s.Value(c.Value)
						end
					end
				else
					j.Field.Field.Text = tostring(c.Value)
				end
			end)
			e.Apply(c, r)
			return r
	...
	function a.G()
		...
		return function(b, c)
			local d, e = a.load("d"), a.load("c")
			local f, g, h, i =
				d.Create, b.__container or b.__instance or b, b.Theme.Controls.RadioButtonGroup, {
					RadioButtons = {},
				}
					...
					f("Frame")({
						...
						f("ImageButton")({
							...
							f("ImageLabel")({
								...
								Image = "rbxassetid://118333468121914",
								Size = UDim2.fromOffset(14, 14),
								__dynamicKeys = { ImageColor3 = h.Overlay[1], ImageTransparency = h.Overlay[2] },
								f("UIGradient")({
									Name = "UIGradient",
									Rotation = 90,
									Transparency = NumberSequence.new({
							...
							f("ImageLabel")({
								...
								__dynamicKeys = { ImageColor3 = h.Dot[1], ImageTransparency = h.Dot[2] },
								__contextKeys = {
									ImageTransparency = function()
							...
							f("ImageLabel")({
								...
								Position = UDim2.fromScale(0.5, 0.5),
								Size = UDim2.fromScale(1, 1),
								__dynamicKeys = { ImageColor3 = h.Stroke[1], ImageTransparency = h.Stroke[2] },
								__contextKeys = {
									ImageTransparency = function()
							...
							f("ImageLabel")({
								...
								Position = UDim2.fromScale(0.5, 0.5),
								Size = UDim2.fromScale(1, 1),
								__dynamicKeys = { ImageColor3 = h.InnerShadow[1], ImageTransparency = h.InnerShadow[2] },
								__contextKeys = {
									ImageTransparency = function()
	...
	function a.H()
		...
		return function(b, c)
			local d, e, f = a.load("d"), a.load("c"), a.load("e")
			local g, h, i, j, k = d.Create, f.UserInputService, f.TweenService, f.Workspace, f.GuiService
			local l, m, n, o, p, q, r, s =
				j.CurrentCamera,
				b.__container or b.__instance or b,
				b.Theme.Controls.MenuButton,
				{ Options = {} },
				-1,
				6,
				6,
			...
			local t, u =
				function()
					o.Menu.AutomaticSize = Enum.AutomaticSize.XY
					task.defer(function()
						local t = o.Menu.AbsoluteSize.Y
						if t >= s then
							o.Menu.AutomaticSize = Enum.AutomaticSize.X
							o.Menu.Size = UDim2.fromOffset(0, s)
						else
							o.Menu.Size = UDim2.fromOffset(0, t)
							o.Menu.AutomaticSize = Enum.AutomaticSize.X
						end
					end)
				end, function()
					local t, u = o.Body, o.Menu
					if not u or not t then
						return
					end
					local v, w, x, y, z = t.AbsolutePosition, t.AbsoluteSize, u.AbsoluteSize, l.ViewportSize
					if c.Maximum and c.Maximum > 1 then
						local A = o.PopUpIndicator or t
						local B = A.AbsolutePosition
						z = B.X + p
					else
						z = v.X + p
					end
					local A, B, C = v.Y + w.Y + q, math.max(r, y.X - r - x.X), math.max(r, y.Y - r - x.Y)
					z = math.clamp(z, r, B)
					A = math.clamp(A, r, C)
					u.Position = UDim2.new(0, z, 0, A)
				end
			local function v(w, x, y)
				local z = (
					g("TextButton")({
						...
						AutoButtonColor = false,
						AutomaticSize = Enum.AutomaticSize.XY,
						BackgroundColor3 = Color3.fromRGB(2, 15, 190),
						BackgroundTransparency = 1,
						BorderColor3 = Color3.fromRGB(0, 0, 0),
						BorderSizePixel = 0,
						Selectable = false,
				...
				local A, B = z:FindFirstChild("Label"), z:FindFirstChild("Checkmark")
				o.Options[y] = z.__instance
				z:GetPropertyChangedSignal("GuiState"):Connect(function()
					if not w.Expanded then
						return
					end
					if z.GuiState == Enum.GuiState.Hover then
						z.BackgroundTransparency = b.Theme.Controls.SelectionFocused[2].Value
						A.TextColor3 = b.Theme.Controls.SelectionFocusedAccent[1].Value
						A.TextTransparency = b.Theme.Controls.SelectionFocusedAccent[2].Value
						B.ImageColor3 = b.Theme.Controls.SelectionFocusedAccent[1].Value
						B.ImageTransparency = b.Theme.Controls.SelectionFocusedAccent[2].Value
					else
						z.BackgroundTransparency = 1
						A.TextColor3 = b.Theme.Text.Primary[1].Value
						A.TextTransparency = b.Theme.Text.Primary[2].Value
						B.ImageColor3 = b.Theme.Text.Primary[1].Value
						B.ImageTransparency = b.Theme.Text.Primary[2].Value
					end
				end)
				z.MouseButton1Click:Connect(function()
					if w then
						if w.Maximum and w.Maximum > 1 then
							local C = w.Value or {}
			...
			c = c or {}
			c.Maximum = c.Maximum or 1
			c.Expanded = c.Expanded or false
			o.Body = (
				e.Apply(
					c,
					g("TextButton")({
						...
						g("Frame")({
							...
							Size = UDim2.fromOffset(16, 16),
							__dynamicKeys = {
								BackgroundColor3 = n.IndicatorBackground[1],
								BackgroundTransparency = n.IndicatorBackground[2],
							},
							g("UICorner")({ Name = "UICorner", CornerRadius = UDim.new(0, 4) }),
							g("ImageLabel")({
								Name = "Indicators",
								BackgroundColor3 = Color3.fromRGB(255, 255, 255),
								BackgroundTransparency = 1,
			...
			o.PopUpIndicator = (o.Body:FindFirstChild("PopUpIndicator"))
			o.CurrentTab = (o.Body:FindFirstChild("Label"))
			o.MenuContainer = (
				g("ScreenGui")({
					Name = "Container",
					IgnoreGuiInset = true,
					...
					g("CanvasGroup")({
						...
						g("ScrollingFrame")({
							...
							__dynamicKeys = {
								BackgroundColor3 = n.MenuBackground[1],
								BackgroundTransparency = n.MenuBackground[2],
							},
							g("UICorner")({ Name = "UICorner", CornerRadius = UDim.new(0, 6) }),
							g("UIStroke")({
								Name = "UIStroke",
								ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
								Transparency = 0.9,
			...
			o.MenuBody = (o.MenuContainer:FindFirstChild("MenuBody"))
			o.Menu = (o.MenuBody:FindFirstChild("PopUpMenu"))
			local w
			local x = {
				Options = function(x)
					for y, z in ipairs(o.Options) do
						z:Destroy()
					end
				...
				Expanded = function(x)
					local y = x == true
					if y then
						o.MenuContainer.Parent = o.Body.__instance
					end
					u()
					local z, A =
						{ GroupTransparency = y and 0 or 1 },
						TweenInfo.new(y and 0 or 0.4, Enum.EasingStyle.Exponential)
					local B = i:Create(o.MenuBody, A, z)
					B:Play()
					B.Completed:Connect(function()
						if w.Expanded == false then
							o.MenuContainer.Parent = nil
						end
					end)
				end,
				Value = function(x)
					local y = w and w.Maximum and w.Maximum > 1
					if y then
						local z = {}
						...
						local A = {}
						for B, C in ipairs(o.Options) do
							if z[B] and C and C:FindFirstChild("Label") then
								table.insert(A, C:FindFirstChild("Label").Text)
							end
						end
						if #A == 0 then
							o.CurrentTab.Text = "None"
						else
							o.CurrentTab.Text = table.concat(A, ", ")
						end
						for B, C in ipairs(o.Options) do
							if not C or not C:FindFirstChild("Checkmark") then
								continue
							end
							if z[B] then
								C:FindFirstChild("Checkmark").Visible = true
								C:FindFirstChild("CheckmarkRepl").Visible = false
							else
								C:FindFirstChild("Checkmark").Visible = false
								C:FindFirstChild("CheckmarkRepl").Visible = true
							end
						end
					else
						if not x or not o.Options[x] then
							o.CurrentTab.Text = "None"
						else
							o.CurrentTab.Text = o.Options[x]:FindFirstChild("Label").Text
						end
						for z, A in ipairs(o.Options) do
							local B = z == x
							if not A or not A:FindFirstChild("Checkmark") then
								continue
							end
							if not B then
								A:FindFirstChild("Checkmark").Visible = false
								A:FindFirstChild("CheckmarkRepl").Visible = true
							else
								A:FindFirstChild("CheckmarkRepl").Visible = false
								if not x then
									A:FindFirstChild("Checkmark").Visible = false
								else
									A:FindFirstChild("Checkmark").Visible = true
								end
							end
						end
					end
			...
			w.Theme = b.Theme
			w.Structures = o
			w.Option = function(y, z)
				local A = #o.Options + 1
				v(w, z, A)
				t()
				table.insert(w.Options, z)
				return o.Options[A]
			end
			w.Remove = function(y, z)
				if z and o.Options[z] then
					o.Options[z]:Destroy()
					w.Options[z] = nil
				end
			end
			e.Apply(c, w)
			o.Body:GetPropertyChangedSignal("AbsolutePosition"):Connect(u)
			o.Body:GetPropertyChangedSignal("AbsoluteSize"):Connect(u)
			o.Menu:GetPropertyChangedSignal("AbsoluteSize"):Connect(u)
			h.InputBegan:Connect(function(y)
				if
					w.Expanded
					and (
						y.UserInputType == Enum.UserInputType.MouseButton1
						or y.UserInputType == Enum.UserInputType.Touch
					)
				then
					local z, A, B = h:GetMouseLocation(), k:GetGuiInset(), o.Menu
					local C, D, E = B.AbsolutePosition, B.AbsoluteSize, z.Y - A.Y
					if z.X < C.X or z.X > C.X + D.X or E < C.Y or E > C.Y + D.Y then
						w.Expanded = false
					end
				end
			end)
			o.Body.MouseButton1Click:Connect(function()
	...
	function a.I()
		...
		return function(b, c)
			...
			local s = function()
				...
				local u, v, w, x = s.AbsolutePosition, s.AbsoluteSize, t.AbsoluteSize, l.ViewportSize
				local y, z, A, B =
					u.X + p, u.Y + (v.Y * 3) + (q * 3), math.max(r, x.X - r - w.X), math.max(r, x.Y - r - w.Y)
				y = math.clamp(y, r, A)
				z = math.clamp(z, r, B)
				t.Position = UDim2.new(0, y, 0, z)
			end
			local function t(u, v, w)
				...
				local y = x:FindFirstChild("Label")
				o.Options[w] = x.__instance
				x:GetPropertyChangedSignal("GuiState"):Connect(function()
					if not u.Expanded then
						return
					end
					if x.GuiState == Enum.GuiState.Hover then
						x.BackgroundTransparency = b.Theme.Controls.SelectionFocused[2].Value
						y.TextColor3 = b.Theme.Controls.SelectionFocusedAccent[1].Value
						y.TextTransparency = b.Theme.Controls.SelectionFocusedAccent[2].Value
					else
						x.BackgroundTransparency = 1
						y.TextColor3 = b.Theme.Text.Primary[1].Value
						y.TextTransparency = b.Theme.Text.Primary[2].Value
					end
				end)
			...
			o.PullDownIndicator = (o.Body:FindFirstChild("PullDownIndicator"))
			o.CurrentTab = (o.Body:FindFirstChild("Label"))
			o.MenuContainer = (
				g("ScreenGui")({
					Name = "Container",
			...
			o.MenuBody = (o.MenuContainer:FindFirstChild("MenuBody"))
			o.Menu = (o.MenuBody:FindFirstChild("PullDownMenu"))
			local u
			local v = {
				Options = function(v)
					for w, x in ipairs(o.Options) do
						x:Destroy()
					end
				...
				Expanded = function(v)
					...
					local x, y =
						...
						TweenInfo.new(w and 0 or 0.4, Enum.EasingStyle.Exponential)
					local z = i:Create(o.MenuBody, y, x)
					z:Play()
					z.Completed:Connect(function()
						if u.Expanded == false then
							o.MenuContainer.Parent = nil
						end
					end)
				end,
				...
				Value = function(v)
					if c.ValueChanged then
						c.Value = v
						task.spawn(c.ValueChanged, u, v)
					end
				end,
			}
			u = e.Wrap(c, v, o.Body)
			u.Type = "PullDownButton"
			u.Theme = b.Theme
			u.Structures = o
			u.Option = function(w, x)
				local y = #o.Options + 1
				t(u, x, y)
				table.insert(u.Options, x)
				return o.Options[y]
			end
			u.Remove = function(w, x)
				if x and o.Options[x] then
					o.Options[x]:Destroy()
					u.Options[x] = nil
				end
			end
			e.Apply(c, u)
			o.Body:GetPropertyChangedSignal("AbsolutePosition"):Connect(s)
			o.Body:GetPropertyChangedSignal("AbsoluteSize"):Connect(s)
			o.Menu:GetPropertyChangedSignal("AbsoluteSize"):Connect(s)
			h.InputBegan:Connect(function(w)
				...
				then
					local x, y, z = h:GetMouseLocation(), k:GetGuiInset(), o.Menu
					local A, B, C = z.AbsolutePosition, z.AbsoluteSize, x.Y - y.Y
					if x.X < A.X or x.X > A.X + B.X or C < A.Y or C > A.Y + B.Y then
						u.Expanded = false
					end
				end
			end)
	...
	function a.J()
		local b, c =
			a.load("c"),
			{
				...
				Toggle = a.load("A"),
				TextField = a.load("B"),
				KeybindField = a.load("C"),
				Slider = a.load("D"),
				Button = a.load("E"),
				Stepper = a.load("F"),
				RadioButtonGroup = a.load("G"),
				PopUpButton = a.load("H"),
				PullDownButton = a.load("I"),
			}
...
local b, c, d, e, f, g, h, i =
	a.load("a"), a.load("b"), a.load("d"), a.load("c"), a.load("e"), a.load("f"), a.load("i"), a.load("J")
local j, k, l, m, n =
	d.Create,
	f.TweenService,
	{ Themes = h, Symbols = g },
...
local function o(p)
	...
	for r, s in pairs(p) do
		local t = typeof(s)
		if t == "table" then
			if s.Value ~= nil and s.Connect and typeof(s.Connect) == "function" then
				q[r] = d.Value(s.Value)
			else
				q[r] = o(s)
			end
...
l.New = function(p)
	...
	local s = (
		j("ImageButton")({
			...
			Parent = r.__instance,
			j("UICorner")({ Name = "UICorner", CornerRadius = UDim.new(1, 0) }),
		})
	)
	s.MouseEnter:Connect(function()
		...
		n = k:Create(s.__instance, m, { ImageTransparency = 0.15 })
		if n then
			n:Play()
		end
	end)
	s.MouseLeave:Connect(function()
		...
		n = k:Create(s.__instance, m, { ImageTransparency = 0.5 })
		if n then
			n:Play()
		end
	end)
	local t = e.Wrap(p, {
		WindowPill = function(t)
			s.Visible = t
		end,
		Theme = function(t)
			local function u(v, w)
				for x, y in pairs(w) do
					if typeof(y) == "table" and typeof(v[x]) == "table" and not y.Value then
						u(v[x], y)
					elseif v[x] and y and y.Value ~= nil then
						v[x].Value = y.Value
					end
				end
			end
	...
	t.Structures = { WindowPill = s }
	for u, v in pairs(i) do
		t[u] = v
	end
	e.Apply(p, t)
	task.defer(e.Apply, p, t)
	return t
end
l.Window = function(p)
	warn([[[Cascade] You cannot call `cascade.Window` directly.
